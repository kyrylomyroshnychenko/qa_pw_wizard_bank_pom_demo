...
test('Assert manager can search customer by First Name', async ({ page }) => {
  ...
  await customersListPage.fillSearchFieldByFirstName(firstName);

  // 3. Assert customer row is present in the table. 

  await customersListPage.assertCustomerPresentByFirstName(firstName);

  // 4. Assert no other rows is present in the table.

  await customersListPage.assertSearchResultsContainOnlyFirstName(firstName);
